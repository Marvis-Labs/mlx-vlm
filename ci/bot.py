...
class DocsChangeOutput:
    """Render the GitHub-hosted documentation validation result."""
    ...
    def sections(self, record: Mapping[str, Any]) -> Sequence[BotSection]:
        checks = [
            check
            for check in _items(record, "checks")
            if check.get("component") == "docs_change"
        ]
        results = [
            result
            for result in _items(record, "results")
            if result.get("component") == "docs_change"
        ]
        errors = [
            error
            for error in _items(record, "errors")
            if error.get("component") == "docs_change"
        ]
