...
def record(
    *, jobs=None, gates=None, checks=None, errors=None, results=None, kind="ci_control"
):
    return {
        "schema_version": 1,
        "kind": kind,
...
def test_docs_change_renders_queued_and_passed_states():
    check = {
        "id": "docs",
        "work_type": "Docs",
    ...
    queued = record(checks=[check])
    queued["components"] = ["docs_change"]
    passed = record(
        checks=[check],
        results=[
            {
    ...
    passed["components"] = ["docs_change"]

    queued_rendered = BotOutput(queued).render()
    passed_rendered = BotOutput(passed).render()

    assert "<strong>Documentation</strong> · DocsChange · Queued" in queued_rendered
    assert (
        "| Documentation | Queued | local links and MkDocs navigation |"
        in queued_rendered
    )
    assert "<strong>Documentation</strong> · DocsChange · Passed" in passed_rendered


def test_docs_change_renders_new_failures():
    value = record(
        results=[
            {
                "component": "docs_change",
                ...
                "findings": {
                    "new_errors": ["docs/guide.md: missing local target missing.md"]
                },
            }
        ]
    )
    ...
    assert "<strong>Documentation</strong> · DocsChange · Test failed" in rendered
    assert "missing local target missing.md" in rendered
